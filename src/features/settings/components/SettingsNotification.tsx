...
const SettingsNotification: React.FC = () => {
  ...
  const tabs: TabItem[] = [
    {
      ...
      icon: "https://c.animaapp.com/jue8RkVm/img/user.svg",
    },
    {
      id: "team-management",
      label: "Team Management",
      icon: "https://c.animaapp.com/jue8RkVm/img/vuesax-linear-profile-2user.svg",
    },
    {
      id: "notification-preferences",
      label: "Notification Preferences",
      icon: "https://c.animaapp.com/jue8RkVm/img/notification-circle.svg",
    },
    {
      id: "privacy-security",
      label: "Privacy & Security",
      icon: "https://c.animaapp.com/jue8RkVm/img/security-lock.svg",
    },
  ];
  ...
  return (
    <div
      ...
      data-model-id="117:567"
    >
      <div className="flex flex-col items-start relative bg-white min-h-screen w-full min-w-0 box-border">
        {/* Navigation Tabs */}
        <nav
          className="flex flex-col items-start justify-center gap-2.5 px-2 py-2 md:px-4 md:p-4 relative w-full min-w-0 box-border"
          role="tablist"
        >
          <div className="flex items-start gap-0.5 p-0.5 bg-neutral-50 rounded-lg w-full md:w-auto md:inline-flex md:p-1 box-border">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                role="tab"
                ...
                className={`inline-flex items-center gap-0.5 px-1 md:px-3 py-1.5 rounded-md flex-1 md:flex-none min-w-0 md:gap-1 box-border ${activeTab === tab.id ? "bg-white shadow-sescy" : ""}`}
                title={tab.label}
              >
                <img
                ...
                />
                <span
                  className={`hidden min-[400px]:inline truncate [font-family:'Archivo',Helvetica] font-normal text-[9px] md:text-sm ${activeTab === tab.id ? "text-black" : "text-[#494949]"}`}
                >
                  {tab.label}
                </span>
              </button>
        ...
        {/* Main Content */}
        <section className="flex flex-col items-start px-3 md:px-4 py-0 w-full min-w-0 box-border">
          {notificationSettings.map((setting) => (
            <div
              key={setting.id}
              className="flex items-center justify-between py-4 w-full min-w-0 border-b border-gray-100 last:border-b-0 box-border"
            >
              <label
                htmlFor={`toggle-${setting.id}`}
                className="flex-1 min-w-0 pr-4 [font-family:'Archivo',Helvetica] font-normal text-[#1a1a1a] md:text-textblacksecondary text-[15px] md:text-sm"
              >
                {setting.label}
              </label>
