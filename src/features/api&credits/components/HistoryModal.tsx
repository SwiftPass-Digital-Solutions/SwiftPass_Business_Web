...
const formatCell = (row: HistoryItem, key: string) => {
  ...
  if (key === "credits") {
    if (row.transactionType === "Usage") return `${row.credits}`;
    if (row.transactionType === "Buy") return `+${row.credits}`;
    return `${row.credits}`;
  }

  if (key === "balance")
    return row.balanceAfter?.toLocaleString?.() ?? row.balanceAfter;
    ...
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm text-gray-900 [font-family:'Archivo',Helvetica]">
            {row.balanceAfter?.toLocaleString?.() ?? row.balanceAfter}
          </span>
        </div>
      </div>
    </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto">
        {pages.map((p, idx) =>
          typeof p === "number" ? (
            ...
            <div
              key={idx}
              className="w-10 sm:w-12 h-10 sm:h-12 flex items-center justify-center text-gray-400 [font-family:'Archivo',Helvetica] text-xs sm:text-sm"
            >
              {p}
            </div>
          ),
...
const TableRow = React.memo(
  ({ row, columns }: { row: HistoryItem; columns: Column[] }) => (
    <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
      {columns.map((column) => (
        <td
          key={column.key}
          className="px-3 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-normal text-gray-700 [font-family:'Archivo',Helvetica] tracking-[-0.42px]"
        >
...
const HistoryModal: React.FC<Props> = ({
  ...
  const wrapperStyle: React.CSSProperties =
    isLarge && sidebarOffset
      ? { marginLeft: sidebarOffset, maxWidth: `calc(100% - ${sidebarOffset})` }
      : {};
    ...
    >
      ...
      >
          ...
          <div className="max-w-7xl mx-auto px-6 py-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-semibold text-gray-900 [font-family:'Archivo',Helvetica]">
                    Billing & History
                  </h2>
                  <p className="text-sm text-gray-500 [font-family:'Archivo',Helvetica]">
              ...
              >
                <span className="font-medium text-white text-sm tracking-[-0.42px] leading-[20.3px] [font-family:'Archivo',Helvetica]">
                  Buy more credits
                </span>
              </button>
            </div>
        ...
        <div className="lg:hidden sticky top-0 bg-white z-10 px-4 py-4">
          ...
          >
            <span className="font-medium text-white text-base [font-family:'Archivo',Helvetica]">
              Buy more credits
            </span>
          </button>
        </div>
              ...
              <thead>
                <tr className="bg-[#fbfbfb] border-b border-gray-200">
                  {columns.map((column) => (
                    <th
                      key={column.key}
                      className="px-3 sm:px-6 py-3 sm:py-4 text-left text-xs sm:text-sm font-normal text-gray-500 [font-family:'Archivo',Helvetica] tracking-[-0.42px]"
                    >
                      {column.label}
                    </th>
                  ))}
                    ...
                    <td colSpan={columns.length} className="px-6 py-16">
                      <div className="flex flex-col items-center justify-center gap-4">
                        <img
                          ...
                          alt="Receipt icon indicating no billing history"
                          src="https://c.animaapp.com/0yfnJNzQ/img/fluent-color-receipt-32.svg"
                          loading="lazy"
                        />
                        <p className="[font-family:'Archivo',Helvetica] font-medium text-gray-900 text-base sm:text-lg tracking-[-0.54px]">
                          No billing history
                        </p>
                      </div>
                    </td>
          ...
          {paged.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-4 py-16">
              <img
                ...
                loading="lazy"
              />
              <p className="[font-family:'Archivo',Helvetica] font-medium text-white text-base tracking-[-0.54px]">
                No billing history
              </p>
            </div>
          ) : (
            <div className="flex flex-col gap-3">{mobileCards}</div>
          )}
